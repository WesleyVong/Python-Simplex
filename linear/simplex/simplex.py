...
def simplex(array, maximize = "max", output = True):
    noprint = False
    ...
    while (True):
        ...
        if case == 1:
            ...
            if len(ratios_list) == 0 and check > 0:
                noprint = True
                failResult[0] = "unbounded feasible"
    ...
    if noprint == False :
        ...
        for i in range(len(t_list)):
            if t_list[i].find("y") == 0:
                org.append((t_list[i], "0"))
            else:
                org.append((t_list[i], b_array[i]))
        for j in range(len(r_list)):
            if r_list[j].find("x") == 0:
                org.append((r_list[j], "0"))
            else:
                org.append((r_list[j], c_array[j]))
        ...
        for i in range(len(org)):
            if org[i][0].find("x") == 0:
                X_list.append(int(org[i][1]))
            elif org[i][0].find("y") == 0:
                Y_list.append(int(org[i][1]))

        result_array = np.append(a_array,[c_array],axis = 0)
        b_array = np.append(b_array,[0])
        result_array = np.concatenate((result_array,np.reshape(b_array,(b_array.size,1))), axis = 1)
        result = {"matrix" : result_array, "values" : org}
        return result
        ...
        b_array = np.append(b_array,[0])
        result_array = np.concatenate((result_array,np.reshape(b_array,(b_array.size,1))), axis = 1)
        result = {"matrix" : result_array, "values" : failResult}
        return result
